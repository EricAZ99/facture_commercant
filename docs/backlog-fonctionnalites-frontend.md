...
- [x] Fonctionnalités activables par commerce (feature flags) — 5 fonctionnalités activables/désactivables individuellement par commerce depuis la fiche détail (`AdminBusinessDetailPage.vue`), indépendamment du plan d'abonnement : Devis, Avoirs, Rapports, Kits (chacune un item de nav + route entière) et Accès API & webhooks (une section dans Paramètres). Effet réel côté commerçant, pas juste un indicateur admin : la navigation masque l'entrée correspondante (barre latérale + palette de commandes), et le routeur redirige vers `/403` en cas d'accès direct par URL (`meta.featureFlag` sur les routes, `authStore.hasFeature()`/`useFeatureFlags()` en miroir du système de permissions existant). Une fonctionnalité activée par défaut à la création d'un commerce ; l'absence du champ (donnée incomplète) est traitée comme "activée", jamais comme un masquage silencieux. Purement informatif côté mock-server (aucune route API n'est bloquée), même limite assumée que les `SubscriptionLimits`. La suite E2E complète a aussi révélé — et corrigé — une régression restée invisible depuis le correctif d'accents FR précédent : plusieurs specs (`auth`, `clients`, `invoices`, `products`, `helpers`) ciblaient encore des libellés sans accent (`Prenom`, `Categorie`, `Telephone`...) devenus obsolètes une fois les accents corrects appliqués à `fr.json` ; la suite n'avait pas été relancée après ce correctif-là. Leçon retenue : toujours relancer l'E2E après une modification de `fr.json`, même une modification "texte seul".
...
- [x] Déploiement Vercel du frontend + du mock-server comme fonction serverless (`api/index.js`, voir `vercel.json`) — plusieurs itérations nécessaires : rewrite SPA, extension `.cjs` puis `.js` + `package.json` dédié dans `api/` pour forcer CommonJS malgré `"type": "module"` racine (Vercel ne reconnaît pas `.cjs` comme fonction), déclaration `functions` explicite avec `maxDuration`.
- [x] Persistance Postgres (Phase 1 "vrai backend") — commerces, utilisateurs et administrateurs plateforme persistés dans Postgres (intégration Neon, Vercel Marketplace, palier gratuit : 0,5 Go / 100h de calcul mensuel), avec mots de passe hachés (bcrypt) et jetons JWT signés (au lieu de jetons opaques dans une `Map` en mémoire). Résout un problème réel découvert en testant le déploiement : sur Vercel, une `Map` en mémoire ne survit pas de façon fiable entre deux requêtes (instances serverless différentes, redémarrages à froid) — l'utilisateur perdait sa session dès le deuxième clic. Architecture volontairement légère (`mock-server/db.js`) : hydrate les tableaux en mémoire existants depuis Postgres au démarrage, les réécrit après chaque requête (aucun des ~40 points de lecture existants n'a besoin d'être touché) ; deux vérifications sensibles au délai (compte actif, commerce suspendu) contournent ce cache et interrogent Postgres **en direct** à chaque requête authentifiée pour une révocation réellement immédiate (prouvé par test réel : désactivation sur une instance, blocage effectif y compris sur une instance tierce jamais informée). Toutes les **autres** entités (clients, produits, factures, devis, avoirs, paiements...) restent volontairement en mémoire pure, hors périmètre de cette phase — voir `mock-server/README.md`.
- [ ] Persistance des entités restantes (clients, produits, factures, devis, avoirs, paiements...) — nécessiterait de convertir chacun des points de lecture/écriture en requêtes SQL (contrairement à la Phase 1, dont la stratégie hydrate/réécrit évite ce travail pour `businesses`/`users`/`platformAdmins`) ; chantier de plusieurs jours, reporté tant que la démo reste acceptée avec des données volatiles pour ces entités.
- [ ] Stockage de fichiers persistant pour les uploads (logo, tampon, pièces jointes) — actuellement redirigé vers `os.tmpdir()` sur Vercel (évite le crash) mais toujours non persistant ; nécessiterait Vercel Blob ou équivalent (S3...).
