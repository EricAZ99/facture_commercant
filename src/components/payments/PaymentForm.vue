...
function validate(): boolean {
  ...
  if (!form.amount.trim() || !Number.isFinite(amount) || amount <= 0) {
    ...
    localErrors.amount = `Le montant ne peut pas depasser le solde restant (${formatCurrency(remaining, props.currency)}).`
  } else {
...
<template>
  <form class="flex flex-col gap-4" @submit.prevent="onSubmit">
    ...
    <div class="grid grid-cols-2 gap-4">
      <BaseInput v-model="form.paidAt" type="date" label="Date du paiement" />
    ...
    <div class="mt-2 flex justify-end gap-2">
      ...
      <BaseButton type="submit" :loading="submitting">Enregistrer le paiement</BaseButton>
    </div>
  </form>
</template>
